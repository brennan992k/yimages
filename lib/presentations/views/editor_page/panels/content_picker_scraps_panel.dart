...
import 'package:yimages/core/_utils/notifications/close_notification.dart';
import 'package:yimages/core/_utils/string_utils.dart';
import 'package:yimages/core/_widgets/mixins/raw_keyboard_listener_mixin.dart';
import 'package:yimages/commands/app/save_image_to_disk_command.dart';
import 'package:yimages/commands/books/create_placed_scraps_command.dart';
import 'package:yimages/commands/books/delete_scraps_command.dart';
import 'package:yimages/core_packages.dart';
import 'package:yimages/data/book_data.dart';
import 'package:yimages/presentations/views/scrap_pile_picker/scrap_pile_picker.dart';
...
class ContentPickerScrapsPanel extends StatefulWidget {
  ...
  @override
  _ContentPickerScrapsPanelState createState() =>
      _ContentPickerScrapsPanelState();
}

class _ContentPickerScrapsPanelState extends State<ContentPickerScrapsPanel>
                ...
                Flexible(
                  child: ScrapPilePicker(
                    ...
                    contextMenuButtons: (ScrapItem scrap) => [
                      ...
                      ContextMenuButtonConfig(
                        "Add To Page",
                        onPressed: isPageSelected
                            ? () => _handleAddPressed(scrap)
                            : null,
                      ),
                      ContextMenuButtonConfig(
                        "Copy Image Address",
                      ...
                      if (enableSaveImageAsBtn)
                        ContextMenuButtonConfig(
                          "Save Image As...",
                          onPressed: () =>
                              SaveImageToDiskCommand().run(scrap.data),
                        ),
                    ],
                  ),
                ),
  ...
  void _handleDeletePressed([ScrapItem? hoverTarget]) async {
    ...
    bool didDelete = await DeleteScrapsCommand()
        .run(bookId: widget.bookId, scrapIds: idList);
    if (didDelete) {
      _scrapPileGridKey.currentState?.clearSelection();
    }
  }

  void _handleScrapPickerChanged(List<ScrapItem> items) =>
